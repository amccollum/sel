...
(function(sel) {
  /* util.coffee
  */
  var attrPattern, checkNth, combinatorPattern, combine, contains, create, eachElement, elCmp, evaluate, extend, filter, filterDescendants, find, findRoots, getAttribute, html, name, nextElementSibling, normalizeRoots, nthPattern, outerParents, parentMap, parse, parseSimple, pseudoPattern, qSA, select, selectorGroups, selectorPattern, synonym, tagPattern, takeElements, _attributeMap, _positionalPseudos, _ref;
  ...
  */
  ...
  pseudoPattern = /::?([-\w]+)(?:\((\([^()]+\)|[^()]+)\))?/g;
  combinatorPattern = /^\s*([,+~])/;
  selectorPattern = RegExp("^(?:\\s*(>))?\\s*(?:/([-\\w]+)/)?\\s*(?:(\\*|\\w+))?(?:\\#([-\\w]+))?(?:\\.([-\\.\\w]+))?((?:" + attrPattern.source + ")*)((?:" + pseudoPattern.source + ")*)(!)?");
  selectorGroups = {
    type: 1,
    idref: 2,
    tag: 3,
  ...
  */
  ...
  find = function(e, roots) {
    ...
    if (e.id || e.idref) {
      ...
      roots.forEach(function(root) {
        var el, id;
        id = e.idref ? getAttribute(root, e.idref) : e.id;
        if (root.getElementById) {
          el = root.getElementById(id);
          if (el && el.id === id) els.push(el);
        } else {
          extend(els, extend([], root.getElementsByTagName(e.tag || '*')).filter(function(el) {
            return el.id === id;
          }));
        }
      });
  ...
  (function() {
    ...
    if (div.firstChild.getAttribute('href') !== '#') {
      _attributeMap['href'] = function(el) {
        return el.getAttribute('href', 2);
      };
      _attributeMap['src'] = function(el) {
  ...
  */
  ...
  checkNth = function(i, val) {
    ...
    if (!val) {
      ...
      a = parseInt((m[1] || '+') + (m[2] === '' ? '1' : m[2]));
      b = parseInt((m[3] || '+') + (m[4] === '' ? '0' : m[4]));
      if (!a) {
        return i === b;
      } else {
