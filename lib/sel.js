(function(sel) {
  /* util.coffee */
  var attrGroups, attrPattern, checkNth, childIndex, combine, comparePosition, contains, elCmp, evaluate, extend, filterAll, filterAttr, filterClasses, filterDescendents, filterPseudo, filterTag, find, findClasses, findId, findTag, html, nextElementSibling, normalizeRoots, nthPattern, parse, parseChunk, parseSimple, pseudoGroups, pseudoPattern, select, selectorGroups, selectorPattern, _differenceMap, _intersectionMap, _unionMap;
  ...
  filterAttr = function(els, name, op, val) {
    ...
    return els.filter(function(el) {
      var attr, value;
      attr = name === 'class' ? el.className : el.getAttribute(name);
      value = attr + "";
      return attr !== null && (!op ? true : op === '=' ? value === val : op === '!=' ? value !== val : op === '*=' ? value.indexOf(val) >= 0 : op === '^=' ? value.indexOf(val) === 0 : op === '$=' ? value.substr(value.length - val.length) === val : op === '~=' ? (" " + value + " ").indexOf(" " + val + " ") >= 0 : op === '|=' ? value === val || (value.indexOf(val) === 0 && value[val.length] === '-') : false);
    });
  };
