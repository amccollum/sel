...
(function(sel) {
  ...
  */
  ...
  outerDescendants = function(els) {
    ...
    filterDescendants(els).forEach(function(el) {
      ...
      if (parent && r[r.length - 1] !== parent) r.push(parent);
    });
    return r;
  };
  ...
  find = function(e, roots) {
    ...
    if (e.id) {
      ...
      roots.forEach(function(root) {
        ...
        if (el && el.id === e.id && contains(root, el)) els.push(el);
      });
      e.id = null;
    } else if (e.name) {
      els = roots.map(function(root) {
        return (root.ownerDocument || root).getElementsByName(e.name).filter(function(el) {
  ...
  */
  ...
  select = document.querySelectorAll ? function(selector, roots) {
    if (!combinatorPattern.exec(selector)) {
      try {
