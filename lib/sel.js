...
(function(sel) {
  /* util.coffee
  */
  var attrPattern, combinatorPattern, combine, contains, create, difference, eachElement, elCmp, evaluate, extend, filter, filterDescendants, find, findRoots, getAttribute, html, intersection, nextElementSibling, normalizeRoots, outerParents, parentMap, parse, parseSimple, pseudoPattern, pseudos, qSA, select, selectorGroups, selectorPattern, tagPattern, takeElements, union, _attrMap;
  ...
  */
  ...
  (function() {
    var checkNth, fn, name, nthMatch, nthMatchPattern, nthPattern, nthPositional, positionalPseudos, _results;
    ...
    nthMatch = function(reversed) {
      return function(els, val, roots) {
        ...
        filtered = els.filter(function(el) {
          return checkNth(el._sel_index, val[1]);
        });
        set.forEach(function(el, i) {
    ...
    for (name in positionalPseudos) {
      ...
      if (name.substr(0, 4) !== 'only') {
        ...
        _results.push(pseudos[name].batch = true);
      } else {
      ...
      switch (e.type) {
        ...
        case '/':
          ...
          if (e.type === ',') {
            ...
            els = els.filter(function(el) {
              var _ref;
              return _ref = el.id, __indexOf.call(ids, _ref) >= 0;
            });
          } else if (e.type === '+') {
  ...
  */
  ...
  return sel.matching = function(els, selector, roots) {
    ...
    if (!e.child && !e.children && !e.pseudos) {
      ...
      return intersection(els, sel.sel(selector, roots || findRoots(els), true));
    }
  };
