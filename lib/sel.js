...
(function(sel) {
  ...
  */
  ...
  pseudoPattern = /(?:::?([-\w]+)(?:\((\([^()]+\)|[^()]+)\))?)/g;
  selectorPattern = RegExp("^\\s*(?:(?:(\\*|\\w+))?(?:\\#([-\\w]+))?(?:\\.([-\\.\\w]+))?(" + attrPattern.source + "*)(" + pseudoPattern.source + "*))(\\s*,|[+~>\\s]+)?");
  selectorGroups = {
    tag: 1,
    id: 2,
    classes: 3,
  ...
  parseSimple = function(type, state) {
    ...
    if (!(m = selectorPattern.exec(rest))) throw new Error("Parse error: " + rest);
    ...
    m.type = type;
    if (m.tag) m.tag = m.tag.toLowerCase();
    if (m.classes) m.classes = m.classes.toLowerCase().split('.');
    if (m.attrsAll) {
      m.attrs = [];
    ...
    m.combinator = !state.left ? '$' : m.combinator.trim() || ' ';
    switch (m.combinator) {
      case ' ':
      case '>':
    ...
    if (roots.length) {
      switch (m.type) {
        ...
        case '>':
          ...
          if (m.not) els = sel.difference(els, find(outerRoots, m.not));
          if (m.child) els = evaluate(m.child, els);
          break;
        case '+':
        case '~':
        case ',':
  ...
  */
  ...
  return sel.sel = function(selector, _roots) {
    var roots;
